 ...
 */
void write_header(std::ofstream &hfile, std::ofstream &dfile, 
    ...
    if (header.version == TAG_VERSION::NEW_22) {
        hfile << "Sample-Rate,"                << int{header.sample_rate} << std::endl
...
void write_short_term(std::ofstream &file,
                      const std::vector<accel_data> &accel,
                      const std::vector<gyro_data> &gyro) {
    auto ai = accel.begin();
    auto gi = gyro.begin();
    while (ai != accel.end()) {
        file << ai->x << ',' << ai->y << ',' << ai->z << ','
