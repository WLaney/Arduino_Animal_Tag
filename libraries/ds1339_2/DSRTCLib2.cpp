...
// reset the register pointer to zero
	I2c.read(DSRTCLib_CTRL_ID, 0x00, 7, rtc_bcd);
}
...
void DSRTCLib::readAlarm(void)
{
	...
	// Connect to the start of the alarm registers, then get
	// secs, min, hr, dow/date
	I2c.read(DSRTCLib_CTRL_ID, uint8_t{DSRTCLib_ARLM1}, 4, rtc_bcd);
	for (int i=0; i<4; i++) {
		// Topmost bit determines how the alarm repeats
		rtc_bcd[i] &= B01111111;
	}
...
{
	I2c.write(DSRTCLib_CTRL_ID, (uint8_t)0x00, rtc_bcd, 7);
...
{
	byte buff[4];
	buff[0] = rtc_bcd[DSRTCLib_SEC] | ((alarm_repeat & B00000001 ) << 7);
	buff[1] = rtc_bcd[DSRTCLib_MIN] | ((alarm_repeat & B00000010 ) << 6);
	buff[2] = rtc_bcd[DSRTCLib_HR] | ((alarm_repeat & B00000100 ) << 5);
	if (alarm_repeat & B00001000) {
		// send DOW as 4th alarm reg byte
		buff[3] = rtc_bcd[DSRTCLib_DOW] | ((alarm_repeat & B00011000 ) << 3); // A1M4 and DY/DT
	} else {
		// send date as 4th alarm reg byte
		buff[3] = rtc_bcd[DSRTCLib_DATE] | ((alarm_repeat & B00011000 ) << 3); // A1M4 and DY/DT		
	}
	I2c.write(DSRTCLib_CTRL_ID, DSRTCLib_ARLM1, buff, 4);
}
...
{
	byte out;
	I2c.read(DSRTCLib_CTRL_ID, registerNumber, 1, &out);
	return out;
}

void DSRTCLib::setRegister(unsigned char registerNumber, unsigned char value)
{
	I2c.write(DSRTCLib_CTRL_ID, registerNumber, &value, 1);
}
